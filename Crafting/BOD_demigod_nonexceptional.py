...
def change_metal():
    ...
    Misc.Pause(500)
    Gumps.SendAction(949095101, metals_dict[ingots])
...
def craft_items():
    ...
    Items.UseItemByID(smith_hammer, 0000)
    Misc.Pause(500)
    Gumps.SendAction(949095101, cat_dict[craft])
    Misc.Pause(250)
    while True:
        worldSave()
        items_added()
        ...
        if Journal.Search('You have worn out your tool!'):
            ...
            Misc.Pause(500)
            Gumps.WaitForGump(949095101, 5000)
        target_item = Items.FindByID(itemid_dict[craft],-1,Player.Backpack.Serial,-1)
        if target_item:
            Items.UseItem(bod)
            Misc.Pause(500)
            Gumps.SendAction(1526454082, 2)
